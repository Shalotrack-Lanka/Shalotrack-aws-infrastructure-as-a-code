...
resource "aws_launch_template" "fleet" {
  ...
  user_data = base64encode(templatefile("${get_terragrunt_dir()}/../scripts/fleet-user-data.sh", {
    ecr_url     = var.ecr_url
    db_host     = var.db_host
    db_port     = var.db_port
  ...
  tag_specifications {
    ...
    tags = { Name = "shalotrack-fleet-management" }
  }
}

resource "aws_autoscaling_group" "fleet" {
  name                = "shalotrack-fleet-asg"
...
inputs = {
  ...
  tg_arn         = dependency.alb.outputs.fleet_tg_arn
  ecr_url        = dependency.ecr.outputs.fleet_repo_url
